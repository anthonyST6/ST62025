// FINAL OVERRIDE: Ensures correct template mapping runs LAST
(function() {
    'use strict';
    ...
    // Force correct templates after a delay to override all other scripts
    function forceCorrectTemplates() {
        const urlParams = new URLSearchParams(window.location.search);
        const subcomponentId = urlParams.get('id') || '1-1';
        const templates = CORRECT_TEMPLATES[subcomponentId] || ["Generic Template 1", "Generic Template 2", "Generic Template 3"];
        
        // Find the resources container
        const resourcesTab = document.getElementById('resources-tab');
        if (!resourcesTab || !resourcesTab.classList.contains('active')) {
            return;
        }
        
        // Find the templates container within resources tab
        const templatesContainer = resourcesTab.querySelector('.templates-list') || 
                                  resourcesTab.querySelector('#resource-templates') ||
                                  resourcesTab.querySelector('.workspace-section');
        
        if (templatesContainer) {
            // Clear and rebuild with correct templates
    ...
    const observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            if (mutation.target.id === 'resources-tab' && 
                ...
                // Force correct templates when resources tab becomes active
                setTimeout(forceCorrectTemplates, 100);
                setTimeout(forceCorrectTemplates, 500);
                setTimeout(forceCorrectTemplates, 1000);
            }
        });
    });
    ...
    // Override ALL tab switching functions
    setTimeout(() => {
    ...
    // Force correct templates on page load if resources tab is active
    document.addEventListener('DOMContentLoaded', () => {
        setTimeout(() => {
            const resourcesTab = document.getElementById('resources-tab');
            if (resourcesTab && resourcesTab.classList.contains('active')) {
                forceCorrectTemplates();
            }
        }, 2000);
    });
    
    // Run periodically to ensure templates stay correct
    setInterval(() => {
        const resourcesTab = document.getElementById('resources-tab');
        if (resourcesTab && resourcesTab.classList.contains('active')) {
            forceCorrectTemplates();
