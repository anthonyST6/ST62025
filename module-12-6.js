...
// Module-specific script for 12-6
(function() {
    console.log('Loading module 12-6');
    ...
    window.loadEducationContent = async function() {
        try {
            const response = await fetch('/api/content/education/12-6');
            if (response.ok) {
    ...
    window.loadWorkspaceFields = function() {
        ...
        if (container && !container.hasChildNodes()) {
            ...
            container.innerHTML = template.generateWorkspaceFields(12, 6);
        }
    };
    ...
    window.loadResources = async function() {
        try {
            const response = await fetch('/api/content/resources/12-6');
            if (response.ok) {
    ...
    window.loadScoreHistory = async function() {
        try {
            const response = await fetch('/api/score-history/12-6');
            if (response.ok) {
    ...
    window.analyzeWorksheet = function() {
        ...
        console.log('Analyzing worksheet for module 12-6');
