...
// Module-specific script for 11-5
(function() {
    console.log('Loading module 11-5');
    
    // Module-specific configurations
    const moduleSettings = {
        blockId: 11,
        subcomponentId: 5,
        apiEndpoint: '/api/analyze/block-11',
        agentType: 'block11Agent'
    };
    
    // Load education content
    window.loadEducationContent = async function() {
        try {
            const response = await fetch('/api/content/education/11-5');
            if (response.ok) {
    ...
    window.loadWorkspaceFields = function() {
        ...
        if (container && !container.hasChildNodes()) {
            ...
            container.innerHTML = template.generateWorkspaceFields(11, 5);
        }
    };
    ...
    window.loadResources = async function() {
        try {
            const response = await fetch('/api/content/resources/11-5');
            if (response.ok) {
    ...
    window.loadScoreHistory = async function() {
        try {
            const response = await fetch('/api/score-history/11-5');
            if (response.ok) {
    ...
    window.analyzeWorksheet = function() {
        ...
        console.log('Analyzing worksheet for module 11-5');
