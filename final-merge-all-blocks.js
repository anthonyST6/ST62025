/**
 * Final merge of ALL blocks 1-16 demo data
 * Combines blocks 1-2, blocks 3-6, and blocks 7-16
 */

const fs = require('fs');

// Read all data sources
const { demoData: currentData } = require('./st6co-demo-data-complete.js');
const blocks3to6 = require('./add-blocks-3-6-demo-data.js');
const blocks7to16Additional = require('./add-custom-demo-data-blocks-7-16.js');
const blocks10to16Remaining = require('./add-remaining-demo-data.js');
const missingSubcomponents = require('./add-missing-subcomponents.js');

// Read blocks 1-2 from restore script
const blocks1to2 = {};
const restoreScript = fs.readFileSync('./restore-blocks-1-6-complete.js', 'utf8');

// Extract blocks 1-2 data from the restore script
// We'll manually add it since it's already defined there
blocks1to2["1-1"] = {
...
// Merge all data sources in priority order
const finalData = {};

// Start with current data (has blocks 7-16)
Object.assign(finalData, currentData);

// Add blocks 1-2
Object.assign(finalData, blocks1to2);

// Add blocks 3-6
Object.assign(finalData, blocks3to6);

// Override with any additional/remaining data
Object.assign(finalData, blocks7to16Additional);
Object.assign(finalData, blocks10to16Remaining);
Object.assign(finalData, missingSubcomponents);
...
for (let block = 1; block <= 16; block++) {
  for (let sub = 1; sub <= 6; sub++) {
    ...
    if (finalData[key]) {
      for (let q = 1; q <= 6; q++) {
        const qKey = `${key}-q${q}`;
        if (finalData[key][qKey]) {
          if (finalData[key][qKey].includes('[Block') && finalData[key][qKey].includes('Demo data')) {
            placeholders++;
          } else {
            complete++;
          }
...
console.log('\n=== Final Merge Status ===');
console.log(`Complete answers: ${complete}/576`);
console.log(`Placeholder answers: ${placeholders}/576`);
console.log(`Completion rate: ${((complete/576)*100).toFixed(2)}%`);
...
const demoData = ${JSON.stringify(finalData, null, 2)};
...
console.log('\n✅ Final demo data file written: st6co-demo-data-complete.js');
console.log('All blocks 1-16 merged successfully!');