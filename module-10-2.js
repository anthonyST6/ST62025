...
// Module-specific script for 10-2
(function() {
    console.log('Loading module 10-2');
    
    // Module-specific configurations
    const moduleSettings = {
        blockId: 10,
        subcomponentId: 2,
        apiEndpoint: '/api/analyze/block-10',
        agentType: 'block10Agent'
    };
    
    // Load education content
    window.loadEducationContent = async function() {
        try {
            const response = await fetch('/api/content/education/10-2');
            if (response.ok) {
    ...
    window.loadWorkspaceFields = function() {
        ...
        if (container && !container.hasChildNodes()) {
            ...
            container.innerHTML = template.generateWorkspaceFields(10, 2);
        }
    };
    ...
    window.loadResources = async function() {
        try {
            const response = await fetch('/api/content/resources/10-2');
            if (response.ok) {
    ...
    window.loadScoreHistory = async function() {
        try {
            const response = await fetch('/api/score-history/10-2');
            if (response.ok) {
    ...
    window.analyzeWorksheet = function() {
        ...
        console.log('Analyzing worksheet for module 10-2');
