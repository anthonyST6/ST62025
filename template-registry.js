/**
 * Template Registry - Central mapping of all 96 subcomponents to their templates
 * 
 * This registry defines:
 * - Template metadata for each subcomponent
 * - Template structure and sections
 * - Customization fields for each template
 ...
 */
function getTemplatesByBlock(blockId) {
    return Object.values(TEMPLATE_REGISTRY).filter(t => t.blockId === blockId);
}
...
/**
 * Get template metadata (without sections)
 */
function getTemplateMetadata(templateId) {
    const template = TEMPLATE_REGISTRY[templateId];
    if (!template) return null;
    
    const { sections, ...metadata } = template;
    return metadata;
}
 ...
 */
function getCustomizableFields(templateId) {
    const template = TEMPLATE_REGISTRY[templateId];
    return template ? template.customizableFields : [];
}

/**
 * Get best practices for a template
 */
function getBestPractices(templateId) {
    const template = TEMPLATE_REGISTRY[templateId];
    return template ? template.bestPractices : [];
}
