/**
 * Complete restoration of blocks 1-6 demo data
 * Extracts from enhance-st6co-answers.js and creates full 6-question format
 */

const fs = require('fs');

// Read current demo data
const { demoData: currentDemoData } = require('./st6co-demo-data-complete.js');

// Create comprehensive blocks 1-6 data
const blocks1to6 = {
  // Block 1: Problem Statement (80-90% maturity)
  "1-1": {
...
// Add blocks 3-6 (will be added in next iteration)
console.log('\nNote: This script currently restores blocks 1-2. Blocks 3-6 need to be added.');
console.log('Current blocks1to6 keys:', Object.keys(blocks1to6));

// Merge with existing data (blocks 7-16 are already good)
const mergedData = Object.assign({}, currentDemoData, blocks1to6);

// Count what we have
let totalSubcomponents = Object.keys(mergedData).length;
let totalQuestions = 0;
let placeholderCount = 0;

Object.keys(mergedData).forEach(subKey => {
  Object.keys(mergedData[subKey]).forEach(qKey => {
    totalQuestions++;
    if (mergedData[subKey][qKey].includes('[Block') && mergedData[subKey][qKey].includes('Demo data')) {
      placeholderCount++;
    }
  });
});
...
console.log(`Actual answers: ${totalQuestions - placeholderCount}`);

// Don't write yet - we need to complete blocks 3-6 first
console.log('\nNot writing file yet - need to complete blocks 3-6 first');
console.log('Run the complete version after adding blocks 3-6 data');