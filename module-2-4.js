...
// Module-specific script for 2-4
(function() {
    console.log('Loading module 2-4');
    ...
    window.loadEducationContent = async function() {
        try {
            const response = await fetch('/api/content/education/2-4');
            if (response.ok) {
    ...
    window.loadWorkspaceFields = function() {
        ...
        if (container && !container.hasChildNodes()) {
            ...
            container.innerHTML = template.generateWorkspaceFields(2, 4);
        }
    };
    ...
    window.loadResources = async function() {
        try {
            const response = await fetch('/api/content/resources/2-4');
            if (response.ok) {
    ...
    window.loadScoreHistory = async function() {
        try {
            const response = await fetch('/api/score-history/2-4');
            if (response.ok) {
    ...
    window.analyzeWorksheet = function() {
        ...
        console.log('Analyzing worksheet for module 2-4');
