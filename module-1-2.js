...
// Module-specific script for 1-2
(function() {
    console.log('Loading module 1-2');
    
    // Module-specific configurations
    const moduleSettings = {
        blockId: 1,
        subcomponentId: 2,
        apiEndpoint: '/api/analyze/block-1',
        agentType: 'block1Agent'
    };
    
    // Load education content
    window.loadEducationContent = async function() {
        try {
            const response = await fetch('/api/content/education/1-2');
            if (response.ok) {
    ...
    window.loadWorkspaceFields = function() {
        ...
        if (container && !container.hasChildNodes()) {
            ...
            container.innerHTML = template.generateWorkspaceFields(1, 2);
        }
    };
    ...
    window.loadResources = async function() {
        try {
            const response = await fetch('/api/content/resources/1-2');
            if (response.ok) {
    ...
    window.loadScoreHistory = async function() {
        try {
            const response = await fetch('/api/score-history/1-2');
            if (response.ok) {
    ...
    window.analyzeWorksheet = function() {
        ...
        console.log('Analyzing worksheet for module 1-2');
